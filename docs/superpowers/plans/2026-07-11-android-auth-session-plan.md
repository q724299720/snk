...
已完成我的页正式账号信息、同步状态、修改密码和主动退出。修改密码成功后无条件清除本地会话并回登录；退出接口网络失败时同样清除本机凭据并给出会话将在下次鉴权失效的提示。账号操作不会访问或删除 Room 草稿，业务导航随顶层认证状态销毁，系统返回键不能回到已退出页面。

| 日期 | 修改人 | 变更范围 | 原因 |
| --- | --- | --- | --- |
| 2026-07-12 | Codex | 标记 Android 认证计划 Task 4 完成 | 补齐账号自助安全操作并固定本地退出优先原则 |
...
登录后的顶层入口会检查旧版匿名安装标识并展示一次性认领说明；用户可以选择“暂不”，且可从“我的”再次进入。服务端成功认领后先写入本地待迁移标记，再原子更新草稿 owner 并标记完成；本地迁移异常将在下次启动恢复，不重复调用服务端。409 已认领只会结束本机提示，不会探测或迁移其他账户的数据。

| 日期 | 修改人 | 变更范围 | 原因 |
| --- | --- | --- | --- |
| 2026-07-12 | Codex | 标记 Android 历史匿名数据认领 Task 6 完成 | 防止升级用户因移除游客模式丢失本机历史数据 |
