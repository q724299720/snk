...
- 注册 JSON 只接受 `username/password`，携带 `deviceId` 或其他未知字段返回 `400`。
- 用户名去除首尾空白后必须为 `3-64` 个 Unicode 字母、数字、点、下划线或连字符；显示保留原始大小写，查重忽略大小写。
- 密码最少 `12` 个字符且最多 `72` 个 UTF-8 字节，使用 cost `12` 的 BCrypt；明文密码和审批票据均不落库、不写日志。
- 审批查询票据使用 256-bit 随机值，服务端只保存 SHA-256 哈希；票据只能调用注册状态查询，不能成为业务身份。
- `POST /api/auth/password/change` 只从可信安全上下文取得当前 userId，成功返回 `204`，递增 `tokenVersion`、清除强制改密标记并撤销全部 Refresh 会话。
- 首 OWNER 仅在系统不存在 OWNER 且同时配置用户名/密码时创建；`SNK_OWNER_FORCE_RESET=true` 才执行紧急重置，并写入不含密码的账号审计。

| 日期 | 修改人 | 变更范围 | 原因 |
| --- | --- | --- | --- |
| 2026-07-11 | Codex | 落地注册、审批状态、修改密码和 Bootstrap OWNER 接口约束 | Task 3 固定严格注册参数、BCrypt 边界和可信用户身份来源 |
