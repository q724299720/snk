...
- 首页公开分享流只能查询 `is_public = true` 且 `deleted_at IS NULL` 的记录
- Android 本地草稿需保存创建时的公开状态，Room v2 通过 `is_public INTEGER NOT NULL DEFAULT 0` 迁移保持老草稿默认私有
- `rating` 表示本次记录事件的体验评分，不表示用户对该 `FoodItem` 的长期总评分
- `rating` 在 MVP 阶段采用 `1-5` 的整数星级
- `comment` 在 MVP 阶段最长 `500` 字符，客户端和服务端都应拒绝超长备注
...
- Bootstrap 首 OWNER 与紧急重置分别记录 `BOOTSTRAP_OWNER / OWNER_FORCE_RESET`；审计快照只记录 Token 版本等非敏感状态。
...
| 2026-07-11 | Codex | 增加 Bootstrap OWNER 审计动作与密码变更会话撤销语义 | Task 3 要求初始化、紧急重置和改密均可追溯且不记录密码 |

## 2026-07-12 搜索封面回填

- 普通记录和快速记录在产品尚无封面且记录为公开状态时，将第一张已上传记录图片写入 `food_items.cover_image_url`。
- `V16__backfill_quick_record_covers.sql` 从最新的公开、未删除记录图片中回填仍为空的产品封面。
- 新建流程与迁移均不得覆盖产品已有封面。

| 日期 | 修改人 | 变更范围 | 原因 |
| --- | --- | --- | --- |
| 2026-07-12 | Codex | 增加快速记录封面同步与 V16 历史回填 | 确保已上传记录图片的产品能在搜索结果中显示图片 |
