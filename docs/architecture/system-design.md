...
- 已增加上传控制器与本地存储服务测试，验证图片写入、缩略图生成与非法文件拒绝行为
...
- 数据库迁移脚本纳入发布流程
- 图片对象存储生命周期管理
- 上传后生成缩略图，并在上传响应中返回 `thumbnailUrl`
...
- 已补充食物条目报错明细记录与后台查询能力：`GET /api/admin/food-items/{foodItemId}/reports`
...
| 2026-06-14 | Codex | 回填图片上传缩略图响应字段 | 当前仓库已落地上传即生成缩略图并返回 `thumbnailUrl` 的资源访问闭环 |
...
- 新记录在进入创建流程时生成稳定 `clientRequestId`，Room、WorkManager 与服务端共同复用；服务端以 `(user_id, client_request_id)` 唯一约束保证幂等。
- 极简记录由服务端单事务完成规范条目匹配、待分类条目创建与记录创建，避免孤立 `FoodItem`。
- Android 草稿允许 `foodItemId`、`rating` 在编辑阶段为空，并将选中图片复制到应用私有草稿目录。
- OCR 继续使用捆绑的 ML Kit 中文模型，客户端保留文本块坐标并提供图片上选择；服务端 OCR 仅作非阻塞兜底。
- 后台治理继续使用 PostgreSQL `pg_trgm` 生成相似候选，并复用现有事务性合并链路，不新增搜索服务。
...
## 2026-07-11 账号服务 Task 3 落地状态

- 已落地注册、审批状态查询、BCrypt cost 12、密码修改和 Bootstrap OWNER 服务。
- 注册审批票据只保存 SHA-256 哈希；随机原文只在注册响应中返回一次，不能构造业务认证主体。
- 密码修改与 OWNER 紧急重置统一递增 `tokenVersion`、撤销当前账号全部 Refresh 会话并写账号审计。
- 密码修改 HTTP 入口通过 `CurrentUser` 读取安全上下文，不接受客户端 userId；Bearer Filter 将在 Task 5 接入，目前未登录请求只能得到 `401`。
- Spring Security 强制鉴权仍未启用，避免 Android 登录能力完成前切断旧 APK；注册接口也暂不部署生产。

| 日期 | 修改人 | 变更范围 | 原因 |
| --- | --- | --- | --- |
| 2026-07-11 | Codex | 回填账号注册、密码管理与 OWNER 初始化实现 | 记录 Task 3 已完成但尚未切换生产强制鉴权的兼容边界 |
